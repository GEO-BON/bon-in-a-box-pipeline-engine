...
2.  Fork the bon-in-a-box-pipelines repository to your personal GitHub repository and then clone it to your computer.

    1.  Documentation on working with forks can be found [here](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo) and cloning [here](https://docs.github.com/en/repositories/creating-and-managing-repositories/cloning-a-repository).

    2.  Make sure you keep the forked repository up to date by adding the bon-in-a-box-pipelines repository as an upstream repository and regularly fetching changes.

3.  Create a branch from the main branch for your pipeline, with a descriptive branch name (e.g. ProtConn_pipeline).

4.  Create your pipeline on this branch.

5.  When it is complete, create a pull request ([instructions here](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request)) to the main branch on the bon-in-a-box-pipelines GitHub and fill out the [peer review template](https://github.com/GEO-BON/bon-in-a-box-pipeline-engine/raw/refs/heads/documentation_peer_review/documentation_website/peer_review_form.docx) , suggesting at least two potential peer reviewers.

6.  When peer review is complete, make any suggested changes to the pipeline and resubmit.

7.  When the pipeline has been accepted, it will be merged to the main branch and published on Zenodo with a DOI.

8.  The BON in a Box team will contact you for any bug fixes or feature requests, and new releases will follow the [versioning structure](pipeline_standards.qmd).
...
To check if the packages you need are already in the default conda environment, go to runners/r-environment.yml or runners/python-environment.yml in the BON in a Box project folder. The list of packages already installed can be found under "dependencies".
...
-   When modifying an existing script, simply save the file and re-run the script from the UI and the new version will be executed.
...
This validates that the syntax and structure are correct, but not that it's content is correct. Hence, peer review of the scripts and the description files is mandatory before accepting a pull requests. Once peer reviewed, it will be tagged as in development, in review, reviewed or deprecated with an optional message describing why it was given that status.
