...
### Specifying R package dependencies with docker
...
-   First, search for the package name on [anaconda](https://anaconda.org/). Packages that are in anaconda will be recognized and easily loaded by conda. If it is there:
    ...
    -   Add it to your local environment, replacing \<package name\> in: `docker exec -it biab-runner-conda mamba install -n rbase <package name>` in the terminal
...
-   The pipeline is generalization (for example can be used with other species, in other regions)
