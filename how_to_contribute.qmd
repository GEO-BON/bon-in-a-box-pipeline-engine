...
6.  Open a pull request to the BON in a Box pipelines repository. The pull request will be peer-reviewed for it’s scientific rigor, it’s reusibility, and the quality of the documentation. While you are waiting, why not contribute by reviewing someone else’s contribution?
...
![](images/Screenshot 2025-02-26 161306.png)
...
Markdown is supported for pipeline, script, input and output descriptions. See this [CommonMark quick reference](https://commonmark.org/help/) for allowed markups. These can include special characters, links, and photos.

### Name and description

YAML files should begin with a name, description, and authors of the script. The name should be the name of the script described by the YAML file. Save the file with the same name as the script that it describes but with a .yml extension.
...
All input and output names should follow the snake case naming conventions. Names should be all lowercase with underscores instead of spaces.
...
Specifying an example will auto-fill that input with the example unless changed by the user. If you do not want there to be an example that auto-fills in the script, write "example: null". This will leave the input blank unless the user adds one. Not specifying any example will cause an error.
...
This means that instead of installing packages in R scripts, they are installed in the R environment in conda. Note that you still need to load the packages in your scripts using the `library()` command.
...
**R**: Here is an example of a conda sub-environment for an R script:
...
**Python:** Here is an example for a python script. You do not need to put anything after conda-forge or before the package names when specifying a conda environment for python.
...
![](images/Screenshot 2025-02-26 154357-01.png)

![](images/Screenshot 2025-02-26 154424.png)
...
![](images/Screenshot 2025-02-26 154455.png)

Below that pane, there is a pane for writing the metadata of the pipeline.

![](images/Screenshot 2025-02-26 154507.png)
...
![](images/Screenshot 2025-02-26 154439.png)
...
*In the event that saving has been disabled on your server instance,* the save button will display "Save to clipboard". To save your modifications:
...
![](images/Screenshot 2025-02-05 111618.png)

You can look at the log of each of your scripts to read errors and debug.

![](images/Screenshot 2025-02-10 034124-02.png)
