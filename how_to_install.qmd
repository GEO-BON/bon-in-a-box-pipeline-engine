...
The user interface allows the user to run and edit pipelines. It is used through a browser, but doesn't need to be installed on a web server. This can all happen locally. The code for the scripts, however, is not edited through the UI but rather with a standard text editor such as VS Code or RStudio.
...
1.  Provide the environment variables: Open the newly cloned repository on your computer
    ...
    d.  Adjust any server option as you see fit.
