...
## Context
	...
	BON in a Box is an open source tool for processing raw data into essential biodiversity variables and indicators. It was developed by the Group on Earth Observations Biodiversity Observation Network (GEO BON) to assist BONs and Parties to the Convention on Biological Diversity in monitoring biodiversity and tracking change. BON in a Box automates analysis workflows into pipelines and allows users to easily parameterize and run analyses without having to code the analyses themselves. Pipelines in BON in a Box are contributed by members of the scientific community. 

This document outlines what kind of pipelines are suitable for BON in a Box, quality control guidelines for contributed pipelines, and the contribution process. 
...
-   Raster data should be in cloud optimized GeoTIFF formats whenever possible. 
...
-   **Point, line, and polygon data** should be in the form of a geopackage.
    ...
    -   Data should be in the coordinate reference system appropriate for the region of interest (to find a crs, search [here](https://epsg.io/))
...
-   The licence of the pipeline (this is chosen by the user, see section on licensing below)
...
Each pipeline should be accompanied by a tutorial in markdown format. This document should have the same name as the pipeline JSON file but with a .md extension and be contained in the same folder as the pipeline it describes. This tutorial should contain a more thorough description of the methods (around 2-3 paragraphs). The tutorial should provide guidance on how to parameterize the pipeline, and walk through a full example analysis. This document should also more thoroughly communicate the limitations of the pipeline in terms of the species, regions, and scales at which it is relevant. See example [here](https://boninabox.geobon.org/indicator?i=ProtConn).
...
-   Any major changes to methods or packages will be a scheduled major release (e.g. 2.0.0). The old pipeline will be labelled as deprecated but will still be available for backwards compatibility.

### **Licensing**

The BON in a Box software is under the [GPLv3](https://www.gnu.org/licenses/quick-guide-gplv3.html) licence, but each pipeline is licensed individually, with the specific license chosen by the contributor. Our recommended license is MIT. You can see differences between open source licenses [here](https://choosealicense.com/licenses/.org/share-your-work/cclicenses/). 
