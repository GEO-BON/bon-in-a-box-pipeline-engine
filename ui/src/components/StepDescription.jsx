      ...
      {codeLink && (
        <p>
          ...
          <a href={codeLink} target="_blank">
            {codeLink.substring(codeLink.search(/(scripts|pipelines)\//))}
          </a>
        </p>
      )}
    ...
    case "CC BY-ND":
      return (
        <a href="https://creativecommons.org/licenses/by-nd/4.0/">
          <img
            alt={license}
            src="https://mirrors.creativecommons.org/presskit/buttons/88x31/png/by-nd.png"
            width="88"
            height="31"
          />
        </a>
...
function getCodeUrl(ymlPath, scriptFileName) {
  ...
  return (
    "https://github.com/GEO-BON/biab-2.0/tree/main/scripts/" +
    removeLastSlash(ymlPath.replaceAll(">", "/")) +
    scriptFileName
  );
}
...
export function InputsDescription({ metadata }) {
  if (!metadata || !metadata.inputs) return null;
...
export function OutputsDescription({ metadata }) {
  if (!metadata || !metadata.outputs) return null;
