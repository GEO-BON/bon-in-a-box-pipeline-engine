...
export const layoutElements = (nodes, edges, callback) => {
  ...
  graph.children = nodes.map((node) => {
    ...
    if (node.type === "io") {
      ...
      if (desc) {
        ...
        if (desc.inputs) {
          // There might not always be inputs
        ...
        console.error(
          "Failed to get description for " + node.data.descriptionFile
        );
      }
    }
  ...
  graph.edges = edges.map((edge) => {
    return {
      ...
      targets: [
        edge.targetHandle
          ? edge.target + ".in." + edge.targetHandle
          : edge.target,
      ],
    };
  });
  ...
  elk
    ...
    .then((result) => {
      ...
      nodes.forEach((reactFlowNode) => {
        const elkjsNode = result.children.find(
          (n) => n.id === reactFlowNode.id
        );
