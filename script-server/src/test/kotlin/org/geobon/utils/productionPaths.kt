...
val productionpipelineRoot: File = File("/pipelines").let {
        // in the docker
        if(it.exists()) it 
...
inline fun <T> withProductionPaths(block: () -> T): T {
    ...
    return withEnvironment("PIPELINES_LOCATION", productionpipelineRoot.absolutePath, OverrideMode.SetOrOverride) {
