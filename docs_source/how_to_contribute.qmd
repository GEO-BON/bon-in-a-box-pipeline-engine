...
**In R,** concurrent installation of packages can be problematic (for example when running the same script multiple times in parallel). For this language only, we provide a function, \`biab_ensure_package(...)\` that monitors the R locking mechanism to hold back the concurrent scripts during installation. Below is an example usage:\
