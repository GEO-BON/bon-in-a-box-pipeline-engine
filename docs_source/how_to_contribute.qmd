...
4.  Create your pipeline on this branch. Make sure it adheres to the [pipeline standards](pipeline_standards.qmd).
...
+------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------------------+
| crsBbox          | Creates a button to open an interface that allows the user to specify a bounding box (geographical minima and maxima) based on an optional selection of country/region and on a CRS, or by clicking on the map to set the bounds of the area of interest for the analysis. |                        |
+------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------------------+
| location         | Full location object. Creates a button to open an interface that allows the user to specify a country, region, coordinate reference system and bounding box (geographical minima and maxima)                                                                               |                        |
+------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------------------------+
...
conda: # Optional: if you script requires specific versions of packages.
  channels:
    ...
    # - r # use this chanel only if some packages are not available in conda-forge
  dependencies:
...
**In R,** concurrent installation of packages can be problematic (for example when running the same script multiple times in parallel). For this language only, you must use the function \`biab_ensure_package(...)\` that monitors the R locking mechanism to hold back the concurrent scripts during installation. Below is an example usage:\
...
When the inputs were entered through [preset selectors](selectors.qmd), the values associated with the selected options are available as a list with values [described here](selectors.qmd). For example, to access the bounding box of an selector with name study_area_bbox of type `crsBbox`. The bounding box can be accessed through:

``` r
bbox <- input$study_area_bbox$bbox
```
...
To access selector values in Julia, the approach is similar to R or Python. For example, to create the CRS ID from the authority and code:
