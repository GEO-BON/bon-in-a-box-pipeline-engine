...
```mermaid
flowchart
    localConda{{Local conda yml?}}
    localPack{{Local conda-pack yml?}}
    remotePack{{Remote conda-pack yml?}}

    mamba[Use mamba]
    usePack[Use conda-pack]
    downloadPack[Download conda-pack]
    resolve[Resolve mamba env]

    Start --> localConda
    localConda -->|Same| mamba
    mamba --> Success

    localPack --> |Different| remotePack
    remotePack -->|Same| downloadPack
    downloadPack --> usePack

    localConda --> |Different| localPack
    localPack -->|Same| usePack
    usePack --> Success


    remotePack --> |Different| resolve
    resolve --> mamba
